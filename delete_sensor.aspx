<%@ Page Language="C#" AutoEventWireup="true" CodeFile="delete_sensor.aspx.cs" Inherits="ThermalCat.Default" %>
<!DOCTYPE html>
<html lang="pt-br">
	<head runat="server">
		<meta charset="utf-8">
    										...
    										<asp:Repeater runat="server" ID="listRepeater">
    											<ItemTemplate>
    												<tr>
    													...
    													<td><%#Eval("Empresa") %></td>
    													<td><a class="fa fa-trash"onclick="return confirm('Todos os dados de temperatura deste sensor serão perdidos!\nDeseja mesmo excluir <%#Eval("Identificador") %>?')" style="font-size: 4vmin;" href="delete.aspx?id=<%#Eval("Id") %>&tabela=tbArduino"></a></td>
    												</tr>
    											</ItemTemplate>
    										</asp:Repeater>
    									</tbody>
		...
		<script>
			$("i").click(function() {
			    if($(event.target).attr('id')=='UserForm'){
			        ...
			        window.location.replace("delete_company.aspx");
			    } else if($(event.target).attr('id')=='SensorForm'){
			        window.location.replace("delete_sensor.aspx");
			    }
			})
